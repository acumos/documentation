...
This guide intended to help Acumos AI Challenge participants with basic info and
links to details about:

* the Acumos platform
* what aspects of the Acumos platform the dev will interact with, through the
  process of submitting models to the challenge

The Acumos Platform
-------------------

The platform devs will be using is described at http://docs.acumos.org/. Since
the docs are a work in progress, you can also see the slides presented at the
Acumos at ONS 2018 launch page. Additional usage details are provided below for
platform aspects that might not be complete/clear from the current docs.
...
First, here is a high-level summary of the process you (as a dev challenge
...
* Select "On-Board Model" and wait a bit, as the workflow icons above go from
  gray to green
...
container image in tar format, for local deployment. You can use the same steps
below to download and test any of the models available in the Acumos public
marketplace. To download a model image:
...
interfaces on HTTP port 3330. The test app for each model should clarify how to
configure the address of the microservice you are testing with.

Launch Your Model in Azure
--------------------------
